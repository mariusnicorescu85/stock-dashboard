  ...
  return (
    <tr className="align-top hover:bg-white/[0.02] transition-colors">
      ...
      <td className="py-4 pr-4 align-top">
        <span className="font-medium leading-snug text-slate-100">{r.name}</span>
        {r.brand ? (
          <span className="mt-0.5 block text-[12px] text-slate-400">{String(r.brand)}</span>
        ) : null}
        <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px]">
          {sigSnoozed ? <span className="text-violet-400/90">Snoozed</span> : null}
          {ordered ? <span className="text-slate-400">Order placed</span> : null}
        </div>
      </td>
      <td
      ...
      <td className="whitespace-nowrap py-4 pr-4 align-top">
        {r.orderByDate && r.orderByDate < briefing.todayYmd ? (
          ...
          <span className="tabular-nums text-slate-400">{monitorQueueFormatDate(r.orderByDate)}</span>
        )}
      </td>
      <td
        className="whitespace-nowrap py-4 pr-4 align-top tabular-nums text-slate-400"
      ...
      <td className="py-4 pr-4 text-right align-top tabular-nums font-semibold text-teal-300/95">
        {r.qtyToOrder}
      </td>
      <td className="hidden py-4 pr-4 text-right align-top tabular-nums text-slate-400 lg:table-cell">
        {chunkUnits}
      </td>
      <td
      ...
      <td className="py-4 pr-4 text-right align-top lg:pr-0">
        <Link
          ...
          className="text-[12px] font-medium text-slate-400 hover:text-teal-400 transition-colors"
        >
          Detail
        </Link>
      </td>
  ...
  return (
    <div className="-mx-4 overflow-hidden sm:-mx-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 lg:px-0">
        ...
        <div
          className="inline-flex rounded-lg border border-slate-700/75 bg-slate-900/60 p-0.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"
          role="group"
      ...
      <div className="max-h-[min(74vh,60rem)] overflow-auto">
        <table className="w-full min-w-[72rem] border-collapse text-[13px]">
          <thead>
            <tr className="sticky top-0 z-10 bg-slate-950 text-left text-[11px] font-medium capitalize tracking-normal text-slate-400 shadow-[inset_0_-1px_0_0] shadow-slate-800/85 [&_th]:pb-3 [&_th]:pt-1">
          ...
          <tbody className="divide-y divide-slate-800/70 text-slate-200">
            {view === "lines"
              ...
              : categoryBlocks.flatMap(({ group, label, runOutBand, soonestDays, units }) => {
                  const ribbon = (
                    <tr key={`cat-${label}`} className="bg-slate-900/95">
                      <th
                        scope="colgroup"
                        colSpan={12}
                        className="border-t border-slate-700/80 px-4 py-2.5 text-left font-semibold lg:px-5"
                      >
                        <span className="block text-[13px] leading-snug text-slate-100">{label}</span>
                        <span className="mt-0.5 block text-[12px] font-normal normal-case tracking-normal text-slate-400">
                          {group.length} line{group.length === 1 ? "" : "s"}
                          {" · "}
                          <span className="tabular-nums text-slate-300">{units.toLocaleString("en-GB")}</span> units
                          {soonestDays != null ? (
                            <>
                              {" · "}Soonest runway{" "}
                              <span className="tabular-nums font-medium text-slate-200">{soonestDays}d</span>
                            </>
                          ) : (
                            ""
                          )}
